...
if (fs.existsSync(componentsPath_react)) {
  ...
  let code = codeBuffer.toString().replace(/import\stype\s\{\s([^}]*)\s\}\sfrom\s'@tarojs\/components-harmony-hybrid[^']*';/ig, `import type { $1 } from '@tarojs/plugin-platform-harmony-hybrid/components-library/dist/types/components';`)
  ...
  if (code.includes('defineCustomElement as define')) {
    code = code.replace(/import\s\{\sdefineCustomElement\sas\sdefine([A-Za-z]+)\s.*/g, '// @ts-ignore\nimport { defineCustomElement$1 as define$1 } from \'@tarojs/plugin-platform-harmony-hybrid/components-library/dist/components\';')
  }
